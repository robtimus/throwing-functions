...
public interface CheckedRunnable<X extends Throwable> {
    ...
    /**
     * Factory method for turning {@code CheckedRunnable}-shaped lambdas into {@code CheckedRunnables}.
     *
     * @param <X> The type of checked exception that can be thrown.
     * @param task The lambda to return as {@code CheckedRunnable}.
     ...
     * allows existing {@link Runnable} instances to be used where {@code CheckedRunnable} is expected.
     *
     * @param <X> The type of checked exception that can be thrown.
     * @param task The task to perform when the returned task is performed.
     * @return A task that wraps any checked exception in an {@link UncheckedException}.
